...
always @(posedge clock, posedge Reset) // finite state machine; if clock or reset rising-edge
	begin
		if (Reset == 1'b1) present_state = reset_state;
			...
			muldiv3: present_state = muldiv4; //Mul Div takes 10 clock cycles
			muldiv4: present_state = muldiv5;
			muldiv5: present_state = muldiv6;
			muldiv6: present_state = muldiv7;
			muldiv7: present_state = fetch0;
			negnot3: present_state = negnot4; //Negate not instructions take 8 clock cycles
			negnot4: present_state = negnot5;
			negnot5: present_state = fetch0;
			rot3: present_state = rot4; //Rotate instructions take 9 clock cycles
			rot4: present_state = rot5;
			rot5: present_state = rot6;
			rot6: present_state = fetch0;
			imm3: present_state = imm4; //Immediate instructions take 9 clock cycles
			imm4: present_state = imm5;
			imm5: present_state = imm6;
			imm6: present_state = fetch0;
			in3: present_state = in4;  //Input output instructions take 7 clock cycles
			in4: present_state = fetch0;
			out3: present_state = out4;
			out4: present_state = fetch0;
			mfhi3: present_state = mfhi4; //Move from instructions take 7 clock cycles
			mfhi4: present_state = fetch0;
			mflo3: present_state = mflo4;
			mflo4: present_state = fetch0;
			sh3: present_state = sh4; //Shift instructions take 9 clock cycles
			sh4: present_state = sh5;
			sh5: present_state = sh6;
			sh6: present_state = fetch0;
			jr3: present_state = jr4; //Jr instructions take 7 clock cycles
			jr4: present_state = fetch0;
			jal3: present_state = jal4; //Jal instructions take 8 clock cycles.
